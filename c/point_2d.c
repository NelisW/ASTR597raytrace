...
int main(int argc, char* argv[])
{
  ...
  double startz,starty,slope,angle,current_z;
  ...
  {				// command line arguments
    ...
    sscanf(argv[5],"%lf",&angle);
  }
  else
  {
    printf("Must supply lens_file_name z0, y0, slope, angle arguments\n");
    exit(1);
  }
  ...
  z[0] = startz;
  y[0] = starty;
  thet[0] = atan(slope) + angle;	// input slope --> angle in radians

  // define -y ray to be one offset below start position; same angle
  z[1] = startz;
  y[1] =  starty;
  thet[1] = atan(slope) - angle;
